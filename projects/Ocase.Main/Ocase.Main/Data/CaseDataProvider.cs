...
namespace Ocase.Main.Data
{
	public class CaseDataProvider : DataProvider
	{
		...
		public List<Case> GetTop100()
		{
			return GetCases(string.Empty, 100);
		}
			...
			using (IDbConnection connection = CreateConnection())
			{
				using (IDbCommand command = connection.CreateCommand())
				{
					command.CommandText = string.Format(caseSelect, 
						joinExpression + (string.IsNullOrEmpty(casePredicate) ? string.Empty : "WHERE " + casePredicate) + (limit > 0 ? (" LIMIT 0," + limit.ToString()) : string.Empty));
